...
import { LayoutGrid, Plus, RotateCcw, Download, Upload } from "lucide-react";
import { useRef } from "react";
import { resetCatalog, exportCatalog, importCatalog } from "@/lib/catalog-store";
...
function AdminLayout() {
  ...
  const onExport = () => {
    ...
    a.href = url;
    a.download = `thunderstream-catalog-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const onImport = async (file: File) => {
    ...
    const res = importCatalog(text);
    if (res.ok) alert(`Imported ${res.count} titles.`);
