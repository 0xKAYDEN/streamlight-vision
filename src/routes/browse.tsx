...
function Browse() {
  ...
  const results = useMemo(() => {
    ...
    return catalog.filter((it) => {
      if (s && !(it.title.toLowerCase().includes(s) || it.synopsis.toLowerCase().includes(s) || it.genres.some(g => g.toLowerCase().includes(s)))) return false;
  ...
  const years = Array.from(new Set(catalog.map((it) => it.year))).sort((a, b) => b - a);
  const allGenres = Array.from(new Set(catalog.flatMap((it) => it.genres))).sort();

  const countText = t("browse.count", {
    n: results.length,
    s: results.length === 1 ? "" : "s",
    q: q ? t("browse.matching", { q }) : "",
  });
  ...
  return (
    <div className="px-4 md:px-8 py-10">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <FilterChip active={!kind} onClick={() => update({ kind: undefined })}>{t("browse.all")}</FilterChip>
